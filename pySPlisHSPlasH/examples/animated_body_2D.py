...
class MyTimeStep(sph.TimeStep):
    def init(self):
        print("INITIALIZED")

    def step(self):
        sim = sph.Simulation.getCurrent()
        n = sim.numberOfFluidModels()
        for i in range(n):
            model = sim.getFluidModel(i)
            for j in range(model.numActiveParticles()):
                #print("Particle", j, "pos", model.getPosition(j))
                model.setPosition(j, model.getPosition(j) + [0, 0.1, 0])
